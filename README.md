...
当前项目更偏向“后端能力演示 + AI 能力扩展骨架”，适合做学习、演示和二次开发，不是直接可上线的生产版。
...
- 基于门店商品的导购推荐
- 基于知识库的轻量 RAG 检索
- AI 问答接口 `/ai/shopping/ask`
- 通义千问不可用时自动回退到本地规则 / 模板生成
...
1. 前端先调用 `/orders/token` 获取下单幂等 token。
2. 提交订单时，服务端用 Redis 校验并消费 token。
3. 根据 `userId + storeId` 找到当前有效购物车。
4. 取出已选中的购物车项。
5. 逐条锁定库存。
6. 创建订单主表、订单项、订单操作日志。
7. 删除已下单的购物车项。
...
1. 接收自然语言导购请求。
2. 通过 `IntentParseProvider` 解析购物意图。
3. 调用门店商品查询工具获取当前门店商品。
4. 按预算、品类、口味、关键词做第一轮打分筛选。
5. 用 `KnowledgeRetriever` 从 `knowledge-base.json` 中检索相关知识片段。
6. 通过 `ShoppingGenerationProvider` 生成推荐理由和推荐总结。
...
3. 查询当前门店商品。
4. 检索知识库片段。
5. 基于“候选商品 + 检索结果”生成问答内容。

## RAG 实现说明

当前版本不是完整向量数据库方案，而是一个适合 Demo 的轻量实现：

- 知识源位于 `src/main/resources/ai/knowledge-base.json`
- 启动时加载到内存
- 按 `storeId / skuId` 做范围过滤
- 再做关键词、标签、中文 n-gram 的简单召回和打分
- 将召回结果作为 AI 生成的上下文

这套实现的价值在于：

- 保留了后续切换到 pgvector / Elasticsearch / Milvus 的扩展点
- 已经具备“检索增强生成”的基础形态
- 对当前 Demo 足够轻量，不引入额外基础设施
